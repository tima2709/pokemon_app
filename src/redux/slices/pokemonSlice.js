...
export const getPokemons = createAsyncThunk(
  ...
  async ({ page, pageSize, name, types }) => {
    const typesOfPokemon = types
      .map((type) => `types%3A${type.toLowerCase()}`)
      .join('%20OR%20');
    const query = getQueryString([
      ...
      {
        key: 'q',
        value: `name:${name.trim().replaceAll(' ', '*')}*+${typesOfPokemon.trim().replaceAll(' ', '*')}`,
      },
    ]);
